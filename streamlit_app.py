...
def run_module1(target_input, source_name):
    ...
    def fetch_ysos_with_pm(target, radius_deg=0.1):
        ...
        if not yso_result: return None, "Module 1 Error: No YSOs found in the primary catalog (II/360)."
        ...
        if not gaia_result: return None, "Module 1 Error: No Gaia DR2 matches found for the YSOs."
        gaia_df = gaia_result[0].to_pandas()
        combined_df = ysos_df.merge(gaia_df, on="Source", suffixes=("", "_GAIA"))
        combined_df = combined_df.rename(columns={"Source": "GAIA_Source_ID", "RA_ICRS": "RA_deg", "DE_ICRS": "DEC_deg", "Jmag": "Jmag", "pmRA": "PMRA_masyr", "pmDE": "PMDEC_masyr", "Epoch": "Epoch_year"})
        return combined_df[["GAIA_Source_ID", "RA_deg", "DEC_deg", "Jmag", "PMRA_masyr", "PMDEC_masyr", "Epoch_year"]], "Success"
...
async def run_module4(grouped_df, summary_df, source_name):
    ...
    def adjust_distances(targets):
        ...
        for i, (theta_i, r_i) in enumerate(valid_targets):
            is_safe = True
            for j, (theta_j, r_j) in enumerate(adjusted_targets):
                distance = sqrt(r_i**2 + r_j**2 - 2 * r_i * r_j * cos(radians(theta_i - theta_j)))
                if distance < MIN_SAFE_DISTANCE: is_safe = False; break
            if is_safe: adjusted_targets.append((theta_i, r_i))
    ...
    for i, group_id in enumerate(grouped_polar_targets.keys()):
        ...
        origin_coord = SkyCoord(group_row['RA_center'], group_row['DEC_center'], unit="deg")
        origin_ra_hms, origin_dec_dms = origin_coord.ra.to_string(unit='hour', sep=':'), origin_coord.dec.to_string(unit='deg', sep=':', alwayssign=True)
...
target_coords = st.sidebar.text_input("Target Coordinates (HH MM SS DD MM SS)", "03 44 30.7 +32 00 17")
source_name = st.sidebar.text_input("Source Name (no spaces)", "IC_348")

st.sidebar.header("Grouping Parameters (Module 2)")
