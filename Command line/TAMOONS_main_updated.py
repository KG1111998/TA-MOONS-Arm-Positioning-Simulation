...
def run_module2(source_name):
    ...
    def simple_collision_check(sublist, min_sep=0.3):
        valid_stars = []
        flagged_stars = []

        # Go through each star in the list
        for i in range(len(sublist)):
            current_star = sublist.iloc[i]
            current_coord = current_star["coord"]

            # Compare with all stars already accepted
            too_close = False
    ...
    valid_groups = []     # Final safe groups
    flagged_list = []     # Problematic stars to retry later

    # Check all sublists initially
    for sublist in initial_sublists:
        valid, flagged = simple_collision_check(sublist)
        if not valid.empty:
